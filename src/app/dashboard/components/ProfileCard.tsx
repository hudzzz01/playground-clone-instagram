...
export default function ProfileCard(props: any) {
  const {sizeAvatar = 10, title, subTitle, buttonName, image, dontShowName, dontShowBtn } = props
  return (
    <div className='flex gap-3'>
      ...
      <div className='flex flex-col justify-center items-start px-2'>
        <div className='font-bold'> {title || "ayunda.Mod99" }</div>
        {!dontShowName && <div> {subTitle || "Asmodeus Ayunda" }</div>}
      </div>
      {!dontShowBtn &&
